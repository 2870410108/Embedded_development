...
void OLED_Scroll_horizontally_OFF(){
	OLED_WriteCommand(0x2E);    //先停止滚动
}
//显示一个字符
...
// 显示数字
void OLED_show_Number(unsigned char y, unsigned char x, float number, unsigned char display_type, unsigned char dis_pattern) {
    ...
    switch (display_type) {
        case 0: // 显示为浮点数
            sprintf(str_num, "%0.3f", number);  // 格式化为浮点数，保留3位小数
            break;
        case 1: // 显示为整数
            sprintf(str_num, "%d", (int)number);  // 格式化为整数
            break;
        case 2: // 显示为十六进制
            sprintf(str_num, "%X", (int)number);  // 格式化为十六进制
            break;
        default:
