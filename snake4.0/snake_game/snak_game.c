...
void menu_control(){
    ...
    if (right) {//右倾就进入子菜单
        ...
        if (right && flag==1){//当菜单标志等于1 && 右倾就进入游戏说明子菜单
			OLED_Clear();
			OLED_showImage(0, 0, 128, 64, image3_128X64);//兰州石化职业技术大学
			flag=3;
		}
...
void isDeath()
{
    ...
    for (i = 1; i < snake_lenght; ++i)
        if (snake_x[0] == snake_x[i] && snake_y[0] == snake_y[i]){// 蛇头与身体重合
			isStart=0;TIM_Cmd(TIM3,DISABLE);//游戏结束，结束定时
		}
}
